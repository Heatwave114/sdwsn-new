        ...
        #        if isinstance(node,Controller) == False:
         #           transition_matrix = node.generate_transition_matrix()
          ...
          #          markov_model = MarkovChain()
           #         predictions = markov_model.predict(node.transitions[-1], transition_matrix, no_predictions=10)
            #        node.predicted_energy_consumed = sum(predictions)
             #       print('predicted energy consumed->',node.predicted_energy_consumed)
                    # print("Predictions: ", node.predicted_energy_consumed)
